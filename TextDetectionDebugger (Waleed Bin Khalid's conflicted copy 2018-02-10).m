...
% imshow(grayImage);
mserRegions = detectMSERFeatures(grayImage,'RegionAreaRange',[6000, 10000]);
...
regionFilteredTextMask(vertcat(connComp.PixelIdxList{[stats.Area] < 1000 | [stats.Area] > 10000})) = 0;
...
ocrtxt = ocr(new, boxes,'CharacterSet','ABCDEFGHIJKLMNOP','TextLayout','Block'); % use the binary image instead of the color image
letter = [ocrtxt.Text];
disp(letter);