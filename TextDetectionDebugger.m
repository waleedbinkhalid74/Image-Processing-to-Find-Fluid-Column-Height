...
% % imshow(grayImage);
% mserRegions = detectMSERFeatures(grayImage,'RegionAreaRange',[50, 300]);
...
% regionFilteredTextMask = edgeAndMSERIntersection;
% 
% %%
...
% figure; imshowpair(gradientGrownEdgeMask, edgeEnhancedMSERMask, 'montage'); 
...
% regionFilteredTextMask = gradientGrownEdgesMask;
% 
% regionFilteredTextMask(vertcat(connComp.PixelIdxList{[stats.Eccentricity] > .995})) = 0;
% regionFilteredTextMask(vertcat(connComp.PixelIdxList{[stats.Area] < 10 | [stats.Area] > 300})) = 0;
% regionFilteredTextMask(vertcat(connComp.PixelIdxList{[stats.Solidity] < .4})) = 0;
% 
% % Visualize results of filtering
% figure; imshowpair(gradientGrownEdgesMask, regionFilteredTextMask, 'montage'); 
...
% cropped = im2bw(imcrop(colorImage, boxes(i,:)));
...
% regionFilteredTextMask = bwmorph(regionFilteredTextMask, 'thicken');
% regionFilteredTextMask = bwmorph(regionFilteredTextMask, 'majority');
% regionFilteredTextMask = bwmorph(regionFilteredTextMask, 'bridge');
% figure;
% imshow(afterStrokeWidthTextMask);

% filled = imfill(mserMask, 'holes'); 
% holes = filled & ~mserMask;
% bigholes = bwareaopen(holes, 200);
% smallholes = holes & ~bigholes;
% new = mserMask | smallholes;
% figure;
% imshow(afterStrokeWidthTextMask);

% figure;
% imtool(afterStrokeWidthTextMask);
% letter = OCR(afterStrokeWidthTextMask);
